...
const CustomDropDown: React.FC<Props> = (props: Props) => {
    return (
        <div className={"custom-drop-down"}>
            ...
            <Field className = {"custom-drop-down-field"} as={"select"} name={props.fieldName} placeHolder={"Something"}>
                {props.options.map((option) => <CustomDropDownOptions option={option} optionValue={"red"}/>)}
            </Field>
        </div>
    );
};
